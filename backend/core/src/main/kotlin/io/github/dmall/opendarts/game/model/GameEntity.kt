...
abstract class GameConfig {
  ...
  @ManyToOne @JoinColumn(name = "starting_player_id") open lateinit var startingPlayer: Player
...
class Game {
  ...
  @Enumerated(EnumType.STRING) @Column(nullable = false) lateinit var gameMode: GameMode
...
class Player {
  ...
  @ManyToMany(mappedBy = "players") var gameSessions: MutableList<GameSession> = mutableListOf()
...
class GameSession {
  ...
  @ManyToOne(optional = false) @JoinColumn(nullable = false) lateinit var game: Game
...
class DartSet {
  ...
  @ManyToOne @JoinColumn(name = "winner_id") var winner: Player? = null
...
class Leg {
  ...
  @ManyToOne(optional = false) @JoinColumn(nullable = false) lateinit var dartSet: DartSet
...
class Turn {
  ...
  @ManyToOne(optional = false) @JoinColumn(nullable = false) lateinit var player: Player
...
class Dart {
  ...
  @Column(nullable = false) var multiplier: Int = 1

  @Column(nullable = false) var autoScore: Boolean = false

  @ManyToOne(optional = false) @JoinColumn(nullable = false) lateinit var turn: Turn

  @get:Transient
  @get:JsonProperty(access = JsonProperty.Access.READ_ONLY)
  val computedScore: Int
    get() = DartScoreUtil.computeScore(score, multiplier)

  @get:Transient
  @get:JsonProperty(access = JsonProperty.Access.READ_ONLY)
  val scoreString: String
    get() = DartScoreUtil.getScoreString(score, multiplier)
}
