...
class X01Game @Autowired constructor(val gameSessionRepository: GameSessionRepository) :
  ...
  private fun getCurrentTurn(currentLeg: Leg, currentPlayer: Player): Turn {
    val existingTurn =
      currentLeg.turns.filter { it.player.id == currentPlayer.id }.lastOrNull { it.darts.size < 3 }
  ...
  private fun getCurrentTurnForRevert(currentLeg: Leg, currentPlayer: Player): Turn =
  ...
  private fun getCurrentScore(config: X01Config, player: Player, currentLeg: Leg): Int {
  ...
  override fun getCurrentGameState(gameSession: GameSession): CurrentGameState {
    ...
    val currentTurnDartsMap =
      gameSession.players.associateWith { player ->
        val currentTurn =
          currentLeg.turns.filter { it.player.id == player.id }.maxByOrNull { it.turnOrderIndex }
