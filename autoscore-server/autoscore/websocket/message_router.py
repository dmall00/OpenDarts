...
from autoscore.util.context import clear_trace_id, set_trace_id
...
class MessageRouter:
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    await self._process_message(websocket, request, trace_id)
    ...
    async def _process_message(self, websocket: ServerConnection, request: BaseRequest, trace_id: str | None = None) -> None:
        ...
        if not handler:
            self.logger.error("Unknown message type: %s", message_type)
            await self._send_error(websocket, f"Unknown message type: {message_type}", request_id)
            return

        await handler.handle(websocket, request, trace_id)
