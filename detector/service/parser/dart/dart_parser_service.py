...
class DartParserService(AbstractYoloParser):
    ...
    def parse(self, detections: List[YoloDetection]) -> List[DartDetection]:
        ...
        if len(dart_detections) > self._config.max_allowed_darts:
            logger.warning("Found %s darts, but only using the first %s", len(dart_detections), self._config.max_allowed_darts)
        if len(dart_detections) == 0:
            logger.warning("No dart detections found in YOLO results")

        for detection in dart_detections[: self._config.max_allowed_darts]:
