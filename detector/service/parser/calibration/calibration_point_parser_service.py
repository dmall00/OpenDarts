...
from detector.model.detection_models import CalibrationPoint, YoloDetection, YoloPoint
...
class CalibrationPointParserService(AbstractYoloParser):
    ...
    def parse(self, detections: List[YoloDetection]) -> List[CalibrationPoint]:
        ...
        for calib_index in range(6):
            ...
            if len(detections) > 1:
                if self._config.enable_smart_calibration_filtering:
                    logger.info(
    ...
    def __calculate_calibration_score(self, detection: YoloDetection, expected_pos: Tuple[float, float], calib_index: int) -> float:
        ...
        max_distance = self._config.calibration_position_tolerance
